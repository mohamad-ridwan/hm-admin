...
export function UseFinishTreatment({
    ...
    const {data: newDataTable, error: errDataTable, isLoading: loadingDataTable} = useSwr(endpoint.getDataTableFinishTreatment(
        searchText,
        currentFilterBy,
    ...
    const dataTableFinishTreatment = newDataTable as DataTableResultT

    if(
        !loadingDataTable &&
        errDataTable
    ){
        throw new AuthRequiredError('A server error occurred. Occurs when retrieving patient data resources finished treatment')
    }
    ...
    function getDataTable():void{
        if(dataTableFinishTreatment?.data){
            setDataColumns(dataTableFinishTreatment.data)
            setGetLastPage(dataTableFinishTreatment.pagination.lastPage)
        }
    }

    useEffect(()=>{
        getDataTable()
    }, [dataTableFinishTreatment])
    ...
    function clickEditFinishTreatment(
        ...
        if (findPatient) {
            ...
            setOnAlerts({
                onAlert: true,
                title: `No patient treatment data found with id "${patientId}"`,
