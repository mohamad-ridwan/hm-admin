...
export function FilterTable() {
    ...
    const {data: dataTable, error: errDataTablePatientRegis, isLoading: loadingDataTable} = useSwr(endpoint.getDataTablePatientRegis(
        searchText,
    ...
    const dataTablePatientRegis: DataTableResultT = dataTable as DataTableResultT

    if(
        !loadingDataTable &&
        errDataTablePatientRegis
    ){
        throw new AuthRequiredError('A server error occurred. occurs when loading the patient registration data resource')
    }
